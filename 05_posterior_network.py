...
**Event features (8-d):** Same as before—z-scored observables + global σ channels—
...
def build_events_8d(
    ...
    ce = chieff.astype(np.float64)

    x0 = (m - nrm["mchirp"]["mean"]) / nrm["mchirp"]["std"]
    x1 = (qd - nrm["q"]["mean"]) / nrm["q"]["std"]
    x2 = (ce - nrm["chieff"]["mean"]) / nrm["chieff"]["std"]
    x3 = (zr - nrm["z"]["mean"]) / nrm["z"]["std"]

    s0 = float(nrm["mchirp"]["std"])
    s1 = float(nrm["q"]["std"])
    s2 = float(nrm["chieff"]["std"])
    s3 = float(nrm["z"]["std"])
    s = np.broadcast_to(np.array([[s0, s1, s2, s3]], np.float64), (len(mchirp), 4))
    return np.hstack(
        [x0[:, None], x1[:, None], x2[:, None], x3[:, None], s]
    ).astype(np.float32)
...
class EmulatorSyntheticCatalogDataset(Dataset):
    """
    (theta, events_8d, mask) for one grid row. Events are **Fresh draws** from
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return theta, torch.from_numpy(x8), torch.from_numpy(m)
...
def main() -> None:
    ...
    csv_p = _resolve(args.hyperparam_csv, root)
    sp_p = _resolve(args.splits_json, root)
    ckpt_dir = _resolve(args.checkpoint_dir, root)

    emu_path = args.emulator_checkpoint
    if emu_path is None:
        emu_path = _resolve(
            DEFAULT_CFM_CKPT if args.emulator == "cfm" else DEFAULT_DIFF_CKPT, root
        )
    else:
        emu_path = _resolve(emu_path, root)
    ...
    plot_d = _HERE / "plots" / "posterior_network" / ts
