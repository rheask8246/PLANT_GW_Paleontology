...
def run_smoke_test(device: str = "cpu", steps: int = 500) -> None:
    ...
    kl_logs: List[Tuple[int, float, float, float, float]] = []  # (step, mchirp_kl, q_kl, chieff_kl, z_kl)
    ...
    # Extended smoke test validation (plots) — run regardless of basic checks
    print("\n" + "=" * 60)
    print("EXTENDED SMOKE TEST VALIDATION")
    print("=" * 60)
...
def run_extended_smoke_test_validation(
    ...
    sspc_data = float(hp_df["chi_b"].max()) < 0.1   # SSPC sfr_a max ≈ 0.035

    if sspc_data:
        # Pick an SMT (most binaries) grid point near the median sfr_a and mu0
        for _ch in ["SMT", "CE", "CHE"]:
            ch_rows = hp_df[hp_df["channel"] == _ch]
            if len(ch_rows) > 0:
                break
    ...
    ax.set_xlabel("Step")
    ax.set_ylabel("Gradient norm (VectorFieldNet)")
    ...
    if sspc_data:
        ...
        fixed_p2 = float(np.median(hp_df["alpha_CE"].dropna()))
    ...
    ax.set_ylabel("Density")
    ax.set_title("Does CFM interpolate smoothly in hyperparameter space?")
    ax.legend()
    plt.tight_layout()
    ...
    # 4a) 2×2 grid of scatter pairs (6 pairs -> 2x3)
    ...
    # -------------------------------------------------------------------------
    # 6. VECTOR FIELD VISUALIZATION (2D SLICE)
    # -------------------------------------------------------------------------
    ...
    with torch.no_grad():
        for i in range(20):
            for j in range(20):
                ...
                v = model.vector_field(x_4d, t_t, context)
    ...
    pass_final_loss = metrics["final_train_loss"] < 0.1
    ...
    summary_lines = [
        "# CFM Smoke Test Validation Summary",
        "",
        "| Metric | Value | Pass? |",
        "|--------|-------|-------|",
    ...
    if sspc_data:
        ...
        fixed_p2b = float(np.median(hp_df["alpha_CE"].dropna()))
