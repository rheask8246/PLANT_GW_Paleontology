...
using ``syntheticstellarpopconvolve`` (Hendriks et al.; GitLab ``dhendriks/syntheticstellarpopconvolve``)
for those two ingredients.  Merger-rate integration over delay times / shells remains
in ``compute_merger_weights`` (same numerics as before).  Sampling: INTRINSIC merger-rate
distribution across redshifts.
...
data/bps_output.h5
    ...
    Spin columns in BPS are ignored for this pipeline: observables are (mchirp, q, z) only.
...
# Synthetic Stellar Pop Convolve (SSPC): MD14 SFR + COMPAS log-skew-normal metallicity.
# Prefer installed package; else load sibling clone (avoids importing package __init__
# which pulls optional deps like voluptuous for ``convolve``).
_SSRC_DIR = REPO_ROOT / "syntheticstellarpopconvolve" / "syntheticstellarpopconvolve"


def _load_sspc_submodule(module_basename: str) -> Any:
    path = _SSRC_DIR / f"{module_basename}.py"
    if not path.is_file():
        raise ImportError(
            f"Missing {path}.  pip install syntheticstellarpopconvolve  OR  "
            "git clone https://gitlab.com/dhendriks/syntheticstellarpopconvolve.git "
            f"next to {REPO_ROOT}."
        ) from None
    spec = importlib.util.spec_from_file_location(f"_plant_sspc_{module_basename}", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load SSPC module from {path}") from None
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def _sspc_madau_and_compas_metal() -> Tuple[
    Callable[..., Any],
    Callable[..., np.ndarray],
]:
    """Load SSPC physics helpers without requiring full package deps (e.g. voluptuous)."""
    if (_SSRC_DIR / "starformation_rate_distributions.py").is_file():
        sfr_mod = _load_sspc_submodule("starformation_rate_distributions")
        metal_mod = _load_sspc_submodule("metallicity_distributions")
        return (
            sfr_mod.madau_dickinson_sfr,
            metal_mod.compas_log_skew_normal_distribution_metallicity_distribution,
        )
    try:
        from syntheticstellarpopconvolve.metallicity_distributions import (
            compas_log_skew_normal_distribution_metallicity_distribution,
        )
        ...
        raise ImportError(
            "Synthetic Stellar Pop Convolve (SSPC) not found. Clone next to this repo:\n"
            "  git clone https://gitlab.com/dhendriks/syntheticstellarpopconvolve.git\n"
            "or pip install syntheticstellarpopconvolve with its requirements.txt (incl. voluptuous)."
        ) from exc
    return madau_dickinson_sfr, compas_log_skew_normal_distribution_metallicity_distribution


madau_dickinson_sfr, compas_log_skew_normal_distribution_metallicity_distribution = (
    _sspc_madau_and_compas_metal()
)

BPS_PATH_DEFAULT = PROJECT_ROOT / "data" / "bps_output.h5"
OUT_PATH_DEFAULT = PROJECT_ROOT / "data" / "sspc" / "models_sspc.hdf5"
...
def build_redshift_grid() -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    `redshifts` includes 0.0 as the first node so cosmology and SFR interpolation use the
    physical low-z limit. ``Z_LOG_FLOOR`` is applied only when writing sampled merger
    redshifts to disk (see main loop), not to this grid.
    """
...
def find_sfr(redshifts: np.ndarray, a: float, b: float,
    ...
    Delegates to syntheticstellarpopconvolve.starformation_rate_distributions.madau_dickinson_sfr.
    """
    q = madau_dickinson_sfr(
        redshifts=np.asarray(redshifts, dtype=np.float64), a=a, b=b, c=c, d=d
    )
    return np.asarray(q.to(u.Msun / u.yr / u.Gpc**3).value, dtype=np.float64)
...
def find_metallicity_distribution(
    ...
    Delegates to syntheticstellarpopconvolve.metallicity_distributions.compas_log_skew_normal_distribution_metallicity_distribution
    (Neijssel+19 / Langer & Norman 2007).
    """
    log_centers = np.arange(min_logZ, max_logZ + step_logZ, step_logZ)
    n_res = max(1, int(round((max_logZ - min_logZ) / step_logZ)))
    dPdlogZ = compas_log_skew_normal_distribution_metallicity_distribution(
        redshifts=np.asarray(redshifts, dtype=np.float64),
        log_metallicity_centers=log_centers,
        mu0=mu0,
        muz=muz,
        sigma_0=sigma0,
        sigma_z=sigmaz,
        alpha=alpha,
        global_logZ_distribution_min=min_logZ,
        global_logZ_distribution_max=max_logZ,
        global_logZ_distribution_res=n_res,
    )
    met_grid = np.exp(log_centers).astype(np.float64)
    dPdlogZ = np.asarray(dPdlogZ, dtype=np.float64)
    p_draw = 1.0 / (max_logZ_bps - min_logZ_bps)
    return dPdlogZ, met_grid, p_draw
