...
_DEFAULT_PLOTS = _HERE / "plots" / "ensemble_posterior"
...
def main() -> None:
    ...
    if args.synthetic_bag:
        ...
        ch = np.array([0.0, 0.05, -0.02, 0.1], np.float32)
        ...
        c_x, c_z = col("chieff", "chi_eff"), col("z", "redshift")
        ...
        ch = df[c_x].values.astype(np.float64)
    ...
    x8 = build_events_8d(
        mch.astype(np.float64), qv.astype(np.float64), ch.astype(np.float64), zv.astype(np.float64), nrm
    )
