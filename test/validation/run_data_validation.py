#!/usr/bin/env python3
"""
Intrinsic data validation suite for PLANT_GW_Paleontology.

This script validates data-generation outputs using the full intrinsic event
range (all_events.parquet), intentionally ignoring detectability weighting.

Outputs:
- test/reports/validation/validation_summary.json
- test/reports/validation/validation_summary.md
- test/reports/validation/*.csv
- test/plots/validation/*.png
"""
...
def check_event_validity(cfg: ValidationConfig, events: pd.DataFrame) -> dict[str, Any]:
    ...
    required = ["mchirp", "q", "chieff", "z", "grid_idx"]
    ...
    violations["nan_or_inf_chieff"] = int((~np.isfinite(events["chieff"])).sum())
    ...
    violations["chieff_out_of_bounds"] = int(((events["chieff"] < -1) | (events["chieff"] > 1)).sum())
