...
Expects CSV columns (case-insensitive): mass_1 / m1, mass_2 / m2, chi_eff / chieff, redshift / z.
Maps to (mchirp, q, chieff, z), builds 8-D features with the **emulator** normalizer
...
build_events_8d = _05.build_events_8d
load_frozen_emulator = _05.load_frozen_emulator

_DEFAULT_OUT = _HERE / "plots" / "gwtc_validation"
...
def main() -> None:
    ...
    p.add_argument("--events-csv", type=Path, required=True, help="CSV with masses, chi_eff, z")
    p.add_argument(
        "--checkpoint-dir",
        type=Path,
        default=_HERE / "checkpoints",
    ...
    c_xi = _find_col(df, ("chieff", "chi_eff"))
    ...
    x8 = build_events_8d(mch, qv, chie, zv, nrm)
    L = x8.shape[0]
    ev = torch.from_numpy(x8).float().view(1, L, 8).to(device)
...
in the 8-D event vector this script feeds to the posterior.

**v1 in this repository:** a **single** run conditions only on
(mchirp, q, χeff, z) and measurement-error structure from the **same** normalizer
